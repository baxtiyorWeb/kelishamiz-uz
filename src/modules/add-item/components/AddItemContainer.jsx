...
const AddItemContainer = () => {
	...
	const fetchCategoryProperties = async (categoryId) => {
		...
		try {
			...
			if (response.data.success) {
				...
				const initialProperties = (response.data.content.properties || []).map((prop) => ({
					propertyId: prop.id,
					type: prop.type,
					value: {
						key: prop.name,
	...
	const handlePropertyChange = (propertyId, value) => {
		setFormData((prev) => ({
			...
			properties: prev.properties.map((prop) =>
				prop.propertyId === propertyId ? { ...prop, value: { ...prop.value, value } } : prop,
			),
		}))
	}
	...
	const onMainImageDrop = useCallback(
		async (acceptedFiles) => {
			...
			try {
				...
				// Update progress
				setUploadingImages((prev) => prev.map((img) => (img.id === uploadId ? { ...img, progress: 100 } : img)))
				...
				setUploadingImages((prev) => prev.map((img) => (img.id === uploadId ? { ...img, error: true } : img)))
	...
	const onAdditionalImagesDrop = useCallback(async (acceptedFiles) => {
		...
		for (const file of acceptedFiles) {
			...
			try {
				...
				// Add the file to our files array
				setImageFiles((prev) => [...prev, { file, isMainImage: false, previewUrl }])
	...
	const removeImage = (index) => {
		...
		setImageFiles((prev) => prev.filter((img) => img.previewUrl !== imageToRemove.file))
		...
		if (imageToRemove.isMainImage && formData.images.length > 1) {
			...
			setFormData((prev) => ({
				...prev,
				images: prev.images.map((img, i) => (i === newMainIndex ? { ...img, isMainImage: true } : img)),
			}))

			setImageFiles((prev) => {
				const firstNonRemovedIndex = prev.findIndex((img) => img.previewUrl !== imageToRemove.file)
				if (firstNonRemovedIndex >= 0) {
					return prev.map((img, i) => (i === firstNonRemovedIndex ? { ...img, isMainImage: true } : img))
				}
				return prev
			})
		}
		...
		<div className="bg-gray-50 min-h-screen py-8">
			<div className="container mx-auto px-4">
				<div className="max-w-full mx-auto">
					<h1 className="text-2xl font-bold text-gray-800 mb-6">Yangi e'lon qo'shish</h1>
					...
					<form onSubmit={handleSubmit} className="space-y-6">
						...
						<div className="bg-white rounded-lg shadow-sm p-6">
							...
							<div className="space-y-6">
									...
									>
										...
										{formData.images.some((img) => img.isMainImage) ? (
											<div className="relative">
												<img
													src={formData.images.find((img) => img.isMainImage)?.file || "/placeholder.svg"}
													alt="Main product"
												...
												<button
													...
													onClick={(e) => {
														e.stopPropagation()
														const mainImageIndex = formData.images.findIndex((img) => img.isMainImage)
														if (mainImageIndex !== -1) {
								...
								{/* Additional Images */}
								<div>
									<label className="block text-sm font-medium text-gray-700 mb-2">Qo'shimcha rasmlar</label>
