...
const ItemCard = React.memo(({ item, index }) => {
	...
	return (
		<div
			className={`group relative flex-shrink-0 rounded-lg shadow-sm hover:shadow-lg transition-all duration-300 overflow-hidden ${index ? "animate-fade-in" : ""
				}`}
		>
			{/* TOP badge */}
			{item?.isTop && (
			...
			<div className="p-4">
				...
				<div className="mb-3">
					<h3 className="text-lg font-medium line-clamp-2 text-gray-800 mb-1">{item?.title}</h3>
				...
				<div className="mb-3">
					<span className="text-lg font-semibold text-emerald-700">
						{item?.price} <span className="text-sm font-normal">{item?.currencyType || "so'm"}</span>
					</span>
					{item?.negotiable && (
						<span className="ml-2 text-xs bg-emerald-50 text-emerald-700 px-2 py-0.5 rounded-full">Kelishiladi</span>
					)}
				</div>
